...
def load_and_crop_img(path, grayscale=False, color_mode='rgb', target_size=None,
    ...
    if target_size is not None:        
        if img.size != (target_width, target_height):
            ...
            if crop not in ["center", "random",'face']:
            ...
            if crop == "center":
                ...
                try:
                    ...
                    (x,y,w,h) = face_detector.CNN_face_detection(img_array)
                    face = np.reshape(img_array[y:y+h, x:x+w], (160,160))
                except ValueError:
                    left_corner = int(round(width / 2)) - int(round(target_width / 2))
                    top_corner = int(round(height / 2)) - int(round(target_height / 2))
                    face = img.crop((left_corner, top_corner, left_corner + target_width, top_corner + target_height))
                return face
